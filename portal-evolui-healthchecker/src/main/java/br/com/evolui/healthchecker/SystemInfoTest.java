...
import br.com.evolui.portalevolui.shared.dto.HealthCheckerSystemInfoDTO;
...
public class SystemInfoTest { // NOSONAR squid:S5786
    ...
    private static void printCpu(CentralProcessor processor) {
        ...
        long sys = ticks[TickType.SYSTEM.ordinal()] - prevTicks[TickType.SYSTEM.ordinal()];
    ...
    private static class ProcessCPU {
        private long id;
        private long time;
    ...
    private void updateProcess() throws Exception {
        int cpuNumber = this.siDTO.getHardware().getProcessor().getLogicalProcessorCount();
        OperatingSystem os = this.si.getOperatingSystem();
        GlobalMemory memory = this.si.getHardware().getMemory();
        double totalMemory = 0.0;
        double totalCPU = 0.0;
        for (int i = 0; i < this.siDTO.getOperatingSystem().getProcesses().size(); ) {
            HealthCheckerSystemInfoDTO.OSProcessDTO x = this.siDTO.getOperatingSystem().getProcesses().get(i);
            if (x.getProcessID() == 0) {
                i++;
                continue;
            }
            OSProcess p = os.getProcess(x.getProcessID());
            if (p == null) {
                this.siDTO.getOperatingSystem().getProcesses().remove(i);
                continue;
            }
            long previousTime = x.getKernelTime() + x.getUserTime();
            long currentTime = p.getKernelTime() + p.getUserTime();
            long timeDifference = currentTime - previousTime;
            long updateDifference = p.getUpTime() - x.getUpTime();
            double cpu = (100d * (timeDifference / ((double) updateDifference)))  / (os.getFamily().equalsIgnoreCase("windows")?cpuNumber:1 );
            double mem = 100d * p.getResidentSetSize() / memory.getTotal();
            x.setMemoryUsagePercent(mem);
            x.setCpuUsagePercent(cpu);
            x.setKernelTime(p.getKernelTime());
            x.setUserTime(p.getUserTime());
            x.setUpTime(p.getUpTime());
            totalMemory += mem;
            totalCPU += cpu;
            i++;
        ...
        System.out.println("CPU Load " + this.siDTO.getHardware().getProcessor().getCpuLoad());
    }
    */

    private static void printProcesses(OperatingSystem os, HardwareAbstractionLayer hal) throws InterruptedException {
        ...
        // Sort by highest CPU
        List<OSProcess> procs = os.getProcesses(ProcessFiltering.VALID_PROCESS, ProcessSorting.CPU_DESC, 300);
        oshi.add("   PID  %CPU %MEM       VSZ       RSS Name");
        double totalMemory = 0.0;
        double totalCPU = 0.0;
        List<ProcessCPU> processCPUS = new ArrayList<>();
        for (int i = 0; i < procs.size(); i++) {
            OSProcess p = os.getProcess(procs.get(i).getProcessID());
            ProcessCPU x = new ProcessCPU();
            x.setId(p.getProcessID());
            x.setTime(p.getKernelTime() + p.getUserTime());
            processCPUS.add(x);
        }
        int cpuNumber = hal.getProcessor().getLogicalProcessorCount();
        System.out.println("cpu number " + cpuNumber);
        Thread.sleep(1000);
        for (int i = 0; i < processCPUS.size(); i++) {
            ProcessCPU x = processCPUS.get(i);
            OSProcess p = os.getProcess((int)x.getId());

            if (p != null) {
                long currentTime = p.getKernelTime() + p.getUserTime();
                long timeDifference = currentTime - x.getTime();
                System.out.println("time diference " + timeDifference);
                ...
                oshi.add(String.format(Locale.ROOT, " %5d %5.1f %4.1f %9s %9s %s", p.getProcessID(),
                        cpu,
                        mem, FormatUtil.formatBytes(p.getVirtualSize()),
        ...
        oshi.add("Total CPU " + totalCPU);
        oshi.add("Total Memory " + totalMemory);
    }
